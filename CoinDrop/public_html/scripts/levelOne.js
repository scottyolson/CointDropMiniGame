//US: 10, Roman: 3, Canadian: 5
COINGAME.levelOne = {};

COINGAME.levelOne.init = function() {
    ...
    //
    COINGAME.levelOne.update = function(elapsedTime, mouse) {
        ...
        if (!COINGAME.textures.clock.done()) {
            ...
            if (Math.floor(COINGAME.prevTime - COINGAME.startTime) % randInt > 0 && Math.floor(COINGAME.prevTime - COINGAME.startTime) % randInt < 4 && coins.length > 0) {
    ...
    //    
    COINGAME.levelOne.render = function(elapsedTime) {
...
var levelOneText = COINGAME.graphics.Text({
            text: 'LEVEL 1!',
    ...
    function foo(){
        ...
        levelOneText.draw();
        console.log(levelOneText);
    }    
    setTimeout(foo, 4000);
    COINGAME.createCountdown();
    COINGAME.prevTime = performance.now();
    COINGAME.startTime = COINGAME.prevTime;
    COINGAME.levelOne.play = true;
    COINGAME.levelOne.score = 0;
    COINGAME.addCoinsBool = false;
    COINGAME.hitCount = 0;
    requestAnimationFrame(gameLoop);
};